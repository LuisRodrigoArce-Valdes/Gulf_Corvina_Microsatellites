...
## Migraine version 0.5.4 (Built on Jun 11 2018 at 11:38:19).
## ...
...
GP$ParameterNames<-c("pGSM","twoNmu") ## Canonical order of elements in parameter vector
GP$paramnbr <- length(GP$ParameterNames) ## fitted or not
GP$timeScale <- "popSize" 
GP$graphicsFormat<-"eps"
...
GP$FONKgNames <- c("pGSM","twoNmu")
GP$FONKgScale <- c("Canonical","Canonical")
names(GP$FONKgScale) <- GP$FONKgNames
GP$samplingSpace<-c("pGSM","twoNmu") ## ONLY for next points
GP$samplingScale<-c("Canonical","Canonical") ## ONLY for next points
...
GP$plotRange<-list()
GP$graphicPars<-list()
GP$plotOptions<-c("NoSurfaces","All1DProfiles")
GP$oneDimCIvars<-c("twoNmu") ## (an empty c() value will be meaningful for nextBounts)
GP$spec1DProfiles<-c()
GP$spec2DProfiles<-c()
...
write("Migraine 0.5.4 (Built on Jun 11 2018 at 11:38:19)",file=GP$cleanResu)
...
write("Data file: C:/Users/Rodrigo/Documents/Archivos/Curvina_Micros/03_Fisheries_Research/Analyses/12_Migraine/07_Curvina_HW-GENEPOP.txt",file=GP$cleanResu)
write("Settings file: migraine_gui.txt",file=GP$cleanResu)
write("",file=GP$cleanResu)
