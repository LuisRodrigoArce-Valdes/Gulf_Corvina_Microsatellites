...
# Longitud MCMC (total de iteraciones)
MCMC <- 10000
...
# Mu [Una sd de 1 es muy grande para una media de 0.0005, PERO msvar no acpeta lognormal negativa, i. e., menor a 1]
mu <- 0.0005
mu.sd <- 10
mu <- data.frame(parameter="Mu", mean=mu, sd=mu.sd)

# t (aquí en años - automaticamente lo pasa a generaciones)
t <- 20000 / gen
t.sd <- 7500 / gen
...
file.copy("../06_Curvina_HW-MSVAR.txt", paste0("../0",i,"_",escenarios[i,1],"/infile"))
